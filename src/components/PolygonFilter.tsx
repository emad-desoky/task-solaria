...
import { Slider, Typography, Box, Select, MenuItem, SelectChangeEvent } from '@mui/material';
...
const PolygonFilter: React.FC<Props> = ({ polygonData }) => {
    ...
    const [priceRange, setPriceRange] = useState<number[]>([0, Math.max(...polygonData.map(d => d.price))]);
    ...
    }, [selectedStatus, priceRange, polygonData]);

    const updatePolygonVisibility = (polygons: NodeListOf<SVGPolygonElement>) => {
        polygons.forEach((polygon) => {
            const dataCode = polygon.getAttribute('data-code');
            if (dataCode) {
                const polygonDataItem = polygonData.find(item => item.code === parseInt(dataCode));

                if (polygonDataItem) {
                    const isVisible = shouldPolygonBeVisible(polygonDataItem);
                    polygon.style.display = isVisible ? 'block' : 'none';
                    polygon.style.fill = isVisible ? STATUS_COLORS[selectedStatus.toLowerCase()] : 'none';
                }
            }
        });
    };

    const shouldPolygonBeVisible = (polygonDataItem: PolygonData): boolean => {
        return (selectedStatus === 'All' || polygonDataItem.status === selectedStatus) &&
