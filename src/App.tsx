...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const manipulatePolygons = () => {
            ...
            polygons.forEach((polygon) => {
                ...
                if (dataCode) {
                    const originalOpacity = polygon.getAttribute('opacity') || DEFAULT_OPACITY;

                    polygon.addEventListener('mouseenter', (event) => handleMouseEnter(event, polygon, dataCode));
                    polygon.addEventListener('mouseleave', () => handleMouseLeave(polygon, originalOpacity));
                }
            });
        };
    ...
    const handleMouseEnter = (event: MouseEvent, polygon: SVGPolygonElement, dataCode: string) => {
        const dataItem = data.find((item: PolygonData) => item.code === parseInt(dataCode));
        if (dataItem) {
            setTooltip({
                visible: true,
                content: dataItem,
    ...
    const handleMouseLeave = (polygon: SVGPolygonElement, originalOpacity: string) => {
